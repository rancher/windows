...
variable "airgap" {
    type = bool
    description = "Configure an airgap network. No outbound connections allowed"
    default = false
}

variable "open_ports" {
    type = list(number)
    description = "Ports to open in the external subnet (the only subnet on simple networks)"
    default = []
}
