<#code used from https://gist.github.com/19WAS85/5424431#>; 
...
while ($listener.IsListening) {
    ...
    $ip=(Get-NetAdapter | Get-NetIpAddress); $header='<html><body><H1>Windows Container Web Server</H1>';
    $callerCountsString='';
    $callerCounts.Keys | % { $callerCountsString+='<p>IP {0} callerCount {1} ' -f $ip[1].IPAddress,$callerCounts.Item($_) };
    $footer='</body></html>';
